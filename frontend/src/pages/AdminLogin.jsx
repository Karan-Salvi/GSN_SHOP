...
export default function AdminLogin() {
  ...
  const [email, setEmail] = useState("admin@gsnfish.com");
  ...
  return (
    <div data-testid="admin-login-page" className="min-h-screen ocean-grid-bg flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        <Link to="/" data-testid="login-brand" className="flex items-center gap-3 mb-8 justify-center">
          <div className="w-12 h-12 rounded-xl bg-ocean-500 flex items-center justify-center text-white">
            <Fish className="w-6 h-6" />
          </div>
          <div className="leading-tight">
            <div className="font-display text-xl font-extrabold text-ocean-600">GSN Fresh Fish</div>
            <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500">Admin Panel</div>
          </div>
        </Link>

        <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-xl shadow-ocean-500/5">
          <h1 className="font-display text-2xl font-extrabold text-slate-900">Owner Login</h1>
          <p className="text-sm text-slate-500 mt-1">दुकानदार लॉगिन · Manage today&apos;s catch and prices</p>
          ...
          <Link to="/" data-testid="back-to-site" className="block text-center mt-6 text-sm text-slate-500 hover:text-ocean-600 transition-colors">
            ← Back to website
          </Link>
        </div>
      </div>
